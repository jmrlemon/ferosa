...
<body class="bg-gray-50 text-gray-900 font-sans antialiased" style="background:#fafafa">
    ...
    <div class="no-print flex flex-wrap items-center justify-between gap-3 mb-5">
      ...
      <div class="flex flex-wrap gap-2">
        <button type="button" onclick="window.print()"
          class="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-4 py-2 text-xs font-medium text-gray-600 hover:bg-gray-50">
          Print
        </button>
        <a href="{{ route('admin.reports.overview-csv', array_filter(['sales_from' => $salesFrom, 'sales_to' => $salesTo])) }}"
           class="inline-flex items-center gap-1.5 rounded-lg bg-gray-900 px-4 py-2 text-xs font-medium text-white hover:bg-gray-800">
          Download CSV
        </a>
      </div>
    </div>

    <section class="report-page bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
      <div class="px-6 py-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Ferosa Landscaping</p>
          <h1 class="text-xl font-bold text-gray-900 mt-1">Revenue Overview Report</h1>
          <p class="text-xs text-gray-400 mt-1">Generated {{ $generatedAt->format('M d, Y h:i A') }}</p>
        </div>
        <div class="text-xs text-gray-500 sm:text-right">
          <p><span class="text-gray-400">From:</span> {{ $salesFrom ?: 'Any' }}</p>
          <p><span class="text-gray-400">To:</span> {{ $salesTo ?: 'Any' }}</p>
        </div>
      </div>

      <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 p-6 border-b border-gray-100">
        <div class="rounded-lg bg-gray-900 text-white p-4">
          <p class="text-[10px] text-gray-300 uppercase tracking-wider">Total Sales</p>
          <p class="text-xl font-bold mt-1">PHP {{ number_format($totalSales, 2) }}</p>
        </div>
        <div class="rounded-lg border border-gray-100 p-4">
          <p class="text-[10px] text-gray-400 uppercase tracking-wider">Total Orders</p>
          <p class="text-xl font-bold mt-1">{{ $totalOrders }}</p>
        </div>
        <div class="rounded-lg border border-gray-100 p-4">
          <p class="text-[10px] text-gray-400 uppercase tracking-wider">Delivered Orders</p>
          <p class="text-xl font-bold mt-1">{{ $deliveredOrders }}</p>
        </div>
        <div class="rounded-lg border border-gray-100 p-4">
          <p class="text-[10px] text-gray-400 uppercase tracking-wider">Pending Orders</p>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-0 border-b border-gray-100">
        <div class="p-6 lg:border-r border-gray-100">
          ...
          <table class="w-full text-xs">
            <thead>
              <tr class="text-left text-gray-400 uppercase tracking-wider border-b border-gray-100">
            ...
            <tbody class="divide-y divide-gray-100">
              ...
              @empty
                <tr><td colspan="3" class="py-5 text-center text-gray-400">No order data.</td></tr>
              @endforelse
            </tbody>
          </table>
        </div>
        ...
        <div class="p-6">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div class="rounded-lg border border-gray-100 p-4">
              <p class="text-[10px] text-gray-400 uppercase tracking-wider">This Week</p>
              <p class="text-lg font-bold text-green-600 mt-1">PHP {{ number_format($thisWeekSales, 2) }}</p>
            </div>
            <div class="rounded-lg border border-gray-100 p-4">
              <p class="text-[10px] text-gray-400 uppercase tracking-wider">Last Week</p>
        ...
        <div class="p-6">
          ...
          <table class="w-full text-xs">
            <tbody class="divide-y divide-gray-100">
              ...
              @empty
                <tr><td class="py-5 text-center text-gray-400">No line items recorded.</td></tr>
              @endforelse
            </tbody>
          </table>
        </div>
